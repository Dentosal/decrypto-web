...
const devReloadCSS = () => {
    ...
    for (var i = 0; i < links.length; i++) {
        ...
        if (link.rel === 'stylesheet') {
            link.href += '?' + Math.random().toString(16).slice(2);
        }
    }
};
...
export default function topbar(state) {
    ...
    <div id="topbar" style="gap: 1rem;">
        ...
        ${
        isLocalhost ? html`<div><input type="button" value="Reload CSS" @click=${(_) => devReloadCSS()}></div>` : null
    }
        ${
        state.user_info?.nick
            ? html`<div>Nick: ${state.user_info.nick} <input type="button" value="✎" @click=${(
                _,
            ) => resetNick(state)}></div>`
